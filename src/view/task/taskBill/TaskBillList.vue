  ...
  <div>
    ...
    <div class="container" v-if="!showEdit">
      <div style="margin:10px">
        ...
        <el-select v-model="userID" placeholder="选择用户">
          <el-option
            v-for="item in userList"
            :key="item.id"
            :label="item['nickname'] != null ? item['nickname'] : '佚名'"
            :value="item.id"
          ></el-option>
        </el-select>
...
export default {
  ...
  async created() {
    ...
    const res2 = await task.getAllUser()
    this.userList = res2
  },
  methods: {
    currentChange(page) {
    ...
    async serachDate() {
      let tasks = await task.getTaskByDate(this.value2, this.typeID, this.userID)
      for (var t in tasks) {
